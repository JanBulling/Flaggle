...
export function getGameStats(): GameStats {
    ...
    const allGuessesEntriies = Object.entries(allGuesses);
    const played = allGuessesEntriies.length;
    ...
    for (const [dayString, guesses] of allGuessesEntriies) {
        bestDistanceSum += Math.min(...guesses.map((guess) => guess.distance));

        const currentDate = DateTime.fromFormat(dayString, "yyyyMMdd");
        ...
        if (won) {
            ...
            if (previousDate == null || previousDate.plus({days: 1}).hasSame(currentDate, "day")) {
                currentStreak++;
            } else {
                currentStreak = 1;
            }
